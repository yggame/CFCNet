        ...
        with torch.cuda.amp.autocast(): 
            if eval_bsize is None:
                ...
                if scale is not None and scale > 4 and cell_decode:
                    pred = batched_predict(model, inp, batch['coord'], batch['cell']*scale/4, eval_bsize)
                else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', default='./configs/test/test-sci1k-10.yaml', type=str, help='config file')
    parser.add_argument('--model', default='./save/metasr_rdn_mlp/epoch_best.pth', type=str, help='model file')
